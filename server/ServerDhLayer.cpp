...
quint64 DhLayer::newMessageId()
{
    quint64 ts = TelegramUtils::formatTimeStamp(QDateTime::currentMSecsSinceEpoch());
    ts &= ~quint64(3);
    ts |= 1; // 1 for reply, 3 for server-initiated messages
...
bool DhLayer::processRequestDHParams(const QByteArray &data)
{
    QByteArray encryptedPackage;
    {
        ...
        if (value != TLValue::ReqDHParams) {
            qWarning() << Q_FUNC_INFO << "Invalid request data" << value.toString();
            return false;
        }
...
bool DhLayer::acceptDhParams()
{
    ...
    //    if ((m_g < 2) || (m_g > 7)) {
    //        qDebug() << "Error: 'g' number is out of acceptable range [2-7].";
    //        return nullptr;
    //    }

    //    if (m_dhPrime.length() != 2048 / 8) {
    //        qDebug() << "Error: dhPrime number length is not correct." << m_dhPrime.length() << 2048 / 8;
    //        return nullptr;
    //    }

    //    if (!(m_dhPrime.at(0) & 128)) {
    //        qDebug() << "Error: dhPrime is too small.";
    //        return nullptr;
    //    }
    ...
    qDebug() << "Taking sha of" << innerData.left(564).toHex();
    const QByteArray sha = Utils::sha1(innerData);
    qDebug() << "Our size:" << sha.size() + innerData.size();
...
bool DhLayer::processSetClientDHParams(const QByteArray &data)
{
    ...
    // answerDcGenOk
    {
        qDebug() << Q_FUNC_INFO << "answerDcGenOk";
        ...
        qDebug() << "NewNonce hash lower..." << newNonceHashLower128Array.toHex();
    }
